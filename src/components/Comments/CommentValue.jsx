        ...
        <div className={`flex flex-col gap-2 p-2 rounded-12`}>
            <div className='flex flex-row justify-between items-center gap-2'>
                <div className='flex flex-row items-center gap-2'>
                    <img src={comment?.userDetails?.photoURL || 'https://cdn-icons-png.flaticon.com/128/1999/1999625.png'} className={`${reply ? 'w-6 h-6' : 'w-8 h-8'} block rounded-full`} />
                    ...
                    {comment?.tags && <p className='font-Golos text-xs font-norma text-brown-50 cursor-pointer'>replied to @{comment?.tags}</p>}
                </div>
            ...
            {reply && <button
                onClick={handleReply}
                className='w-fit border border-black-500 ml-4 px-2 py-[2px] z-10 rounded-full text-black-500 font-Golos text-[10px] font-normal leading-4 cursor-pointer'
            >
                Reply
            </button>}
