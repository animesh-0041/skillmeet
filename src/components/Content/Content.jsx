    ...
    return (
        <React.Fragment>
            {(() => {
                switch (block.type) {
                    case 'header':
                        return <h2 className='w-full font-Golos text-3xl font-bold text-black-700' dangerouslySetInnerHTML={{ __html: block.data.text }} />;
                    case 'paragraph':
                        return <p className='w-full font-Golos text-sm font-normal text-[#5b5b5b]' dangerouslySetInnerHTML={{ __html: block.data.text }} />;
                    case 'image':
                        return (
                            <div className={`w-full h-auto flex flex-col gap-2 items-center ${ImageSizeFormatHandler({ withBorder: block.data.withBorder, withBackground: block.data.withBackground, stretched: block.data.stretched })}`}>
                                ...
                                {block.data.file.caption &&
                                    <p className='font-Golos text-base font-semibold text-[#5b5b5b]' dangerouslySetInnerHTML={{ __html: block.data.caption }} />
                                }
                            </div>
                        );
                    case 'inlineImage':
                            ...
                            <div className='w-full flex flex-col gap-2 ml-6'>
                                {block.data.items.map((item, index) => (
                                    <div key={index} className='w-full flex flex-row items-center gap-3'>
                                        <p className={`${block.data.style === 'ordered' ? "min-w-6 text-base font-normal" : 'font-Golos text-2xl font-semibold'} flex flex-row items-center text-black-900`}>{block.data.style === 'ordered' ? `${index + 1}.` : '•'}</p>
                                        <p className='font-Golos text-base font-normal text-black-700' dangerouslySetInnerHTML={{ __html: item }} />
                                    </div>
                                ))}
                            </div>
                        );
                    ...
                    case 'quote':
                        return (
                            <blockquote className='w-full flex flex-col gap-2'>
                                <p
                                    className='font-Golos text-base font-normal px-4 py-3 text-[#4a4949] bg-[#d3d3d32f] border-l-[4px] border-[#ccc]'
                                    dangerouslySetInnerHTML={{ __html: block.data.text }}
                                />
                                {block.data.caption &&
                                    <footer
                                        className='border rounded px-4 py-3 font-Golos text-base font-normal text-[#5b5b5b]'
                    ...
                    case 'warning':
                        return (
                            <div className='w-full flex flex-col gap-2'>
                                {block.data.title &&
                                    <h3
                                        className='font-Golos text-base font-normal px-4 py-3 text-[#4a4949] bg-[#ffa7a717] border-l-[4px] border-[#ff8282]'
                                        dangerouslySetInnerHTML={{ __html: block.data.title }}
                                    />
                                }
                                {block.data.message &&
                                    <p
                                        className='font-Golos text-base font-normal px-4 py-3 text-[#4a4949] bg-[#ffa7a717]'
                            ...
                            <div className='w-full px-4 py-1'>
                                {block.data.items.map((item, index) => (
                                    <div className='w-full flex flex-row gap-4 items-center' key={index}>
                                        ...
                                        <span className='font-Golos text-sm font-normal text-[#5b5b5b]'>{item.text}</span>
                                    </div>
                                ))}
                            </div>
                        );
                                            ...
                                            return (<tr className='border' key={rowIndex}>
                                                {row.map((cell, cellIndex) => (
                                                    <td
                                                        key={cellIndex}
                                                        dangerouslySetInnerHTML={{ __html: cell }}
                                                        className='border font-Golos text-start px-3 py-[6px] overflow-hidden text-[#5b5b5b]'
                                                    />
                                                ))}
                                            </tr>)
                                        }
