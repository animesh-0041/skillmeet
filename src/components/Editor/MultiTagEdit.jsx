            ...
            <>
                {profileBarHandler()?.map((item, ind) => (
                    <Button
                        ...
                        className={`w-fit ${profileBar ? 'px-4 text-xs' : 'px-3 text-sm'} py-[6px] flex flex-row justify-center items-center gap-2 rounded-full font-Golos whitespace-nowrap capitalize shadow-sm shadow-black-300 ${ButtonActiveState(item)}`}
                    >
                        {item}
                    </Button>
                ))}
