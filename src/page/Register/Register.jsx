...
export const Register = () => {
    ...
    const validButton = (props) => {
        if (props === 'NEXT') {
            ...
            return (!registerDetails.name || !registerDetails.email || !registerDetails.password || !registerDetails.confirmPassword);
        } else {
            return true;
        }
    }
        ...
        await createUserWithEmailAndPassword(auth, email, password)
            ...
            .catch((error) => {
                if (error.code === 'auth/email-already-in-use') {
                    dispatch({ type: 'SET_EMAIL_ERROR', payload: 'Account is arealy exist, Login Please.' });
                    dispatch({ type: 'SET_PASSWORD_ERROR', payload: 'Account is arealy exist, Login Please.' });
                }
                // loading model off
                closeLoadingModel();
            });
        ...
        switch (props) {
            case 'NEXT':
                return (
                    ...
                    validateEmail({ email: registerDetails.email, dispatch: dispatch }) && setStep(1)
                );
            case 'BACK':
                return setStep(0);
            default:
    ...
    }, [registerDetails.email, registerDetails.password, registerDetails.confirmPassword, isValidPassword, isValidEmail])
        ...
        <div className={`w-full h-screen flex relative flex-col text-left shrink-0 gap-6 bg-black-0`}>
            <div className='w-full h-screen border flex flex-row justify-between'>
                <div className='w-full max-w-[600px] relative flex justify-center items-center gap-10'>
                    ...
                    <h2 className='font-Golos font-bold text-2xl leading-7 text-blue-950'>Create an account</h2>
                        {
                            inputs && inputs?.map((item, index) => (
                                <InputBox
                                    ...
                                    errorMsg={registerDetails.isPasswordError || registerDetails.isEmailError}
                                />
                            ))
                        }
                        ...
                        <div className='w-full flex flex-col gap-4'>
                            {(step !== 0 && registerDetails.isPasswordError) && <Paragraph center color={'text-red-500'}>{registerDetails.isPasswordError}</Paragraph>}
                            {(step === 0 && registerDetails.isEmailError) && <Paragraph center color={'text-red-500'}>{registerDetails.isEmailError}</Paragraph>}
                                ...
                                if (btn === "BACK") {
                                    return (
                                        <Button
                                            ...
                                            className={'w-full px-4 py-2 rounded border text-white bg-black-900 hover:bg-opacity-90 active:bg-opacity-70'}
                                        >
                                            <Paragraph color={'text-white'} type={'semiheading'}>{btn}</Paragraph>
                                        </Button>
                                    )
                                } else {
                                    return (
                                        <Button
                                            ...
                                            onClick={() => handleRegister(btn)}
                                            className={`w-full px-4 py-2 rounded text-white bg-black-900 ${validButton(btn) ? "bg-opacity-80" : "hover:bg-opacity-90 active:bg-opacity-70"}`}
                                        >
                                            <Paragraph color={'text-white'} type={'semiheading'}>{RegisterLoading ? 'Loading...' : btn}</Paragraph>
                                        </Button>
                                    )
                                }
                            })}
                        ...
                        <Paragraph center>
                            ...
                            <PageNavigation url={'/login'}>
                                <Paragraph color={'text-blue-950 border-b-[1px] border-blue-950'}>Login Here</Paragraph>
                            </PageNavigation>
                        </Paragraph>
                    </div>
                </div>

                <div className='w-full border bg-black-50'>
